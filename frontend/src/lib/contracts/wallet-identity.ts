 ...
 * `require_auth()`). Returns the transaction hash. Errors if the wallet is
 * already registered to a different commitment (CommitmentConflict) or the same
 * one (AlreadyInGroup).
 */
export async function registerWallet(
  ...
  return invokeContractCall(
    NETWORK.contractIds.walletIdentity,
    'register_wallet',
    [new Address(wallet).toScVal(), commitmentScVal(commitment)],
    wallet,
  )
}
